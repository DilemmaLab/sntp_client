...
#include <netdb.h>  
  ...
  ntpdate() {  
	char  *hostname="91.226.136.136";
  	int  portno=123;
  	//NTP- порт 123  int  maxlen=1024;
  	// размер буфера int  i;
     	unsigned  /*  Построение сообщения длиной 48 байт, в котором все байты = 0, кроме  версии протокола 100, т.е. 4 */  
	char  msg[48]={100,0,0,0,0,0,0,0,0};
	//  отправляемый пакет unsigned  long  buf[ maxlen];
  	//  буфер приема struct protoent *proto;
  	struct sockaddr_in server_addr;
  	int  s;
   	//  соккет int    tclock1,  tclock2=0;
	//  время на компьютере int  tmit;
  	//  время сервера proto=getprotobyname("udp");
  	...
  	tclock1 =clock();
  	//Время на компьютере до отправки сообщения printf("Clock before sending request = %d\ n",tclock1);
  	//  Отправка данных i=sendto(s,msg,sizeof(msg),0,(struct sockaddr  *)&server_addr,sizeof(server_addr));
  	tclock2=clock();
 	//Время на компьютере после отправки сообщения printf("Clock after sending request = %d\ n",tclock2);
  	printf("Program spent %d ms on request (RTT)\ n",tclock2- tclock1);
  	//  получение ответных данных i=recv( s,buf,   sizeof( buf),0);
	...
	tmit=ntohl((time_t)buf[10]);
  	...
  	// Первая строка буфера приема  - интерпретация при помощи побитовых  операций int val;
  	val=( ( buf[0] ) >> 30);
  ...
  /*  Конвертация времени  NTP- сервера в стандартное время  Unix/ Linux- системы  - для этого необходимо вычесть из полученного времени число  секунд между  0000  UT   1  Января 1900  ( точка отсчета времени  NTP- сервера ) и  0000  UT   1  Января  1970  ( точка отсчета времени  Unix/ Linux- системы ) и  применить функцию  ctime(..)  */  
  tmit- = 2208988800U;
